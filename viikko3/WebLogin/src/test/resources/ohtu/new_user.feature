Feature: A new user account can be created if a proper unused username and password are given
    ...
    Scenario: creation fails with too short username and valid password
        Given command new user is selected
        When too short username "my" and valid password "myymyy10" are entered
        Then user is not created and error "username should have at least 3 characters" is reported  

    Scenario: creation fails with correct username and too short password
        Given command new user is selected
        When valid username "nipsu" and too short password "n" are entered
        Then user is not created and error "password should have at least 8 characters" is reported

    Scenario: creation fails when password and password confirmation do not match
        Given command new user is selected
        When valid username "vilijonkka" and password "vvvvvvvv8" and password confirmation "vvvvvvvv9" are entered
